...
export default function Home() {
  ...
  /* --- RENDER FUNCTION FOR EDITABLE FIELDS --- */
      ...
      <div className="w-1/3 p-6 border-r border-gray-300 bg-white shadow-lg sticky top-0 h-screen overflow-y-auto">
        ...
        <div className="mb-8 border-b border-gray-200 pb-6">
          ...
          <p className="text-sm text-gray-700 mb-4">
            <strong>Direct Edit:</strong> Click on any single text field (Name, Dates, Title) on the resume to update it instantly.<br />
            <strong>Multi-line Edit:</strong> Use the sections below for descriptions and achievements.
          </p>
        </div>
            ...
            {resume.experience.map((job, jobIndex) => (
                <div key={jobIndex} className="bg-gray-50 p-3 rounded-lg mb-4 border border-gray-200 relative">
                    <p className="text-xs font-semibold text-indigo-600 mb-2">
                      Entry #{jobIndex + 1}: {job.title} at {job.company}
                    </p>
                    <button 
        ...
        <div className="mb-8 border-b border-gray-200 pb-6">
            ...
            {resume.projects.map((project, projIndex) => (
                <div key={projIndex} className="bg-gray-50 p-3 rounded-lg mb-4 border border-gray-200 relative">
                    <p className="text-xs font-semibold text-indigo-600 mb-2">
                      Entry #{projIndex + 1}: {project.name}
                    </p>
                    <button 
        ...
        <div className="mb-8 border-b border-gray-200 pb-6">
            ...
            {resume.education.map((edu, eduIndex) => (
                <div key={eduIndex} className="bg-gray-50 p-3 rounded-lg mb-4 border border-gray-200 relative">
                    <p className="text-xs font-semibold text-indigo-600 mb-2">
                      Entry #{eduIndex + 1}: {edu.institution}
                    </p>
                    <button 
        ...
        <div className="mb-6 pt-6 border-t border-gray-200">
            ...
            <div className="flex gap-4">
                <button onClick={() => setPaperBg("paper-bg-white")} className={`w-10 h-10 rounded-full border-4 ${paperBg === 'paper-bg-white' ? 'border-gray-500 ring-4 ring-gray-200' : 'border-gray-200'}`} style={{ backgroundColor: '#ffffff', outline: '1px solid #ccc' }} title="White Background"></button>
                <button onClick={() => setPaperBg("paper-bg-light-gray")} className={`w-10 h-10 rounded-full border-4 ${paperBg === 'paper-bg-light-gray' ? 'border-gray-500 ring-4 ring-gray-200' : 'border-gray-200'}`} style={{ backgroundColor: '#f3f4f6' }} title="Light Gray Background"></button>
            </div>
        </div>
        ...
        <div className="mb-6 pt-6 border-t border-gray-200">
            ...
            <div className="flex gap-2 flex-wrap">
                <button onClick={() => setTheme("default")} className={`w-10 h-10 rounded-full border-4 ${theme === 'default' ? 'border-indigo-500 ring-4 ring-indigo-200' : 'border-gray-200'}`} style={{ backgroundColor: '#6366f1' }} title="Indigo Theme"></button>
                <button onClick={() => setTheme("theme-blue")} className={`w-10 h-10 rounded-full border-4 ${theme === 'theme-blue' ? 'border-blue-500 ring-4 ring-blue-200' : 'border-gray-200'}`} style={{ backgroundColor: '#3b82f6' }} title="Blue Theme"></button>
                <button onClick={() => setTheme("theme-teal")} className={`w-10 h-10 rounded-full border-4 ${theme === 'theme-teal' ? 'border-teal-500 ring-4 ring-teal-200' : 'border-gray-200'}`} style={{ backgroundColor: '#14b8a6' }} title="Teal Theme"></button>
                <button onClick={() => setTheme("theme-red")} className={`w-10 h-10 rounded-full border-4 ${theme === 'theme-red' ? 'border-red-500 ring-4 ring-red-200' : 'border-gray-200'}`} style={{ backgroundColor: '#ef4444' }} title="Red Theme"></button>
                <button onClick={() => setTheme("theme-purple")} className={`w-10 h-10 rounded-full border-4 ${theme === 'theme-purple' ? 'border-purple-500 ring-4 ring-purple-200' : 'border-gray-200'}`} style={{ backgroundColor: '#a855f7' }} title="Purple Theme"></button>
            </div>
        </div>
      </div>
